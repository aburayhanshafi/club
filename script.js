...
const logoZoomSlider = document.getElementById('logo-zoom-slider');
...
const LOGO_BOX = { x: 595, y: 465, w: 430, h: 290, radius: 20 };
...
logoZoomSlider.addEventListener('input', (e) => { logoScale = parseFloat(e.target.value); draw(); });
    ...
    reader.onload = (event) => {
        ...
        img.onload = () => {
            if (type === 'photo') {
                ...
                clubLogo = img;
                // Logo scaling (fit inside box, rather than cover)
                const scaleX = (LOGO_BOX.w - 40) / img.width;
                const scaleY = (LOGO_BOX.h - 40) / img.height;
                logoScale = Math.min(scaleX, scaleY);
                logoX = LOGO_BOX.x + (LOGO_BOX.w - img.width * logoScale) / 2;
                logoY = LOGO_BOX.y + (LOGO_BOX.h - img.height * logoScale) / 2;
                
                logoZoomSlider.min = logoScale * 0.2;
                logoZoomSlider.max = logoScale * 4;
                logoZoomSlider.value = logoScale;
...
function startDrag(e) {
    ...
    } else if (x >= LOGO_BOX.x && x <= LOGO_BOX.x + LOGO_BOX.w && y >= LOGO_BOX.y && y <= LOGO_BOX.y + LOGO_BOX.h) {
        if (clubLogo) {
            activeLayer = 'logo';
            isDragging = true;
            startX = x - logoX;
...
function drag(e) {
    ...
    if (activeLayer === 'photo' && userPhoto) {
        photoX = x - startX;
        photoY = y - startY;
    } else if (activeLayer === 'logo' && clubLogo) {
        logoX = x - startX;
        logoY = y - startY;
    }
...
function draw() {
    ...
    // 4. Draw Club Logo (Erasing the old one first)
    if (clubLogo) {
        ctx.save();
        // Erase old logo by drawing a white rounded rect over it
        drawRoundedRect(ctx, LOGO_BOX.x, LOGO_BOX.y, LOGO_BOX.w, LOGO_BOX.h, LOGO_BOX.radius);
        ctx.fillStyle = '#ffffff';
        ctx.fill();
        
        // Clip so logo doesn't spill out
        ctx.clip();
        
        ctx.translate(logoX, logoY);
        ctx.scale(logoScale, logoScale);
        ctx.drawImage(clubLogo, 0, 0);
        ctx.restore();
        
        // Re-draw border if needed? The original has a rounded rect border?
        // It's just a white box, but if logo has white bg it blends.
    }
}
